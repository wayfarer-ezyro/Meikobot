...
from skylee.__main__ import GDPR, STATS, USER_INFO
from skylee.modules.disable import DisableAbleCommandHandler
from skylee.modules.helper_funcs.alternate import send_action, typing_action
...
def bin(update, context):
    ...
    if not text:
        ...
        return
    try:
        results = get(f"https://lookup.binlist.net/{text}").json()
        reply_text = f'Bin {text}\nDetails: {results["scheme"]}'
        reply_text += f'\n\nExample: {results["brand"]}'
    except IndexError:
        reply_text = (
            f"Word: {text}\nResults: Sorry could not find any matching results!"
        )
...
BIN_HANDLER = DisableAbleCommandHandler("bin", bin)
